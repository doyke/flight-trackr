function [registres, plots] = update_registres2(registres, plots, trames, lon_ref, lat_ref)
    % Contrôle CRC
    ...
    if (sum(cond))
        ...
        [r(cond).vitesse_air] = deal(vitesse_air{:});
        [r(cond).vitesse_sol] = deal(vitesse_sol{:});
        [r(cond).cap] = deal(cap{:});
    ...
    if (sum(cond))
        ...
        [r(cond).latitude] = deal(latitudes{:});
        [r(cond).longitude] = deal(longitudes{:});
        [r(cond).altitude] = deal(altitudes{:});
        [r(cond).cprFlag] = deal(CPR{:});
    ...
    if (~isempty(id))
        plots = update_plots2(plots, registres, id);
    end
end
