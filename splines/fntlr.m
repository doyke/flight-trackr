...
if nargin>3  % check that both X and INTERV are of the correct size
    ...
    if length(sizex)~=2||any(sizex-[m,1])
        error(message('SPLINES:FNTLR:wrongsizex', num2str( m )))
    end
    if ~iscell(interv) % convert to cell, if possible
        sizei = size(interv);
        if any(sizei-[m,2])
            error(message('SPLINES:FNTLR:wrongsizeinterv', num2str( m )))
        end
        interv = num2cell(interv,2);
    end
    if length(interv)~=2
        error( message( 'SPLINES:FNTLR:PolynomialOnlyForBivariate' ) );
    end
    warning( message( 'SPLINES:FNTLR:TaylorPolynomialToBeRemoved' ) );
end
...
if nargin>3
    ...
    taylor = reshape(taylor,[d,dorder]);
    maxdorder = max(dorder);
